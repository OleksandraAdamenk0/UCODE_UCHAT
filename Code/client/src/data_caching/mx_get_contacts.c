//
// Created by maksym_ch on 11.12.24.
//

#include "data_exchange.h"
#include "connection.h"
#include "responses.h"
#include "logger.h"
#include "libmx.h"
#include "cJSON.h"

static char *get_contacts_request(long long int id) {
    cJSON *json = cJSON_CreateObject();
    if (!json) {
        logger_error("Failed to create JSON object for user settings request\n");
        return NULL;
    }

    cJSON_AddStringToObject(json, "action", "get_contacts");
    cJmakeSON_AddNumberToObject(json, "user_id", id);

    char *json_str = cJSON_PrintUnformatted(json);
    cJSON_Delete(json);
    return json_str;
}

static t_response *parse_contacts_response(char *data) {
    if (!data) {
        logger_error("Response is NULL\n");
        return NULL;
    }
    cJSON *json = cJSON_Parse(data);
    if (!json) {
        logger_error("Failed to parse response JSON\n");
        return NULL;
    }
    cJSON *status = cJSON_GetObjectItemCaseSensitive(json, "status");
    cJSON *msg = cJSON_GetObjectItemCaseSensitive(json, "message");
    if (!cJSON_IsNumber(status)) {
        logger_error("Response JSON does not contain a valid status\n");
        cJSON_Delete(json);
        return NULL;
    }

    t_response *response = malloc(sizeof(t_response));
    if (!response) {
        logger_error("Failed to allocate memory for response\n");
        cJSON_Delete(json);
        return NULL;
    }
    response->action = GET_CONTACTS;
    response->status = (int)cJSON_GetNumberValue(status);
    response->msg = msg && cJSON_IsString(msg) ? mx_strdup(cJSON_GetStringValue(msg)) : NULL;
    response->data = NULL;

    if (response->status < 0) {
        cJSON_Delete(json);
        return response;
    }

    cJSON *count_json = cJSON_GetObjectItemCaseSensitive(json, "count");
    cJSON *array = cJSON_GetObjectItemCaseSensitive(json, "contacts");
    if (!cJSON_IsArray(array)) {
        logger_error("Response JSON does not contain a valid contacts array\n");
        cJSON_Delete(json);
        response->status = -1;
        return response;
    }
    if (!cJSON_IsNumber(count_json)) {
        logger_error("Response JSON does not contain a valid counter\n");
        count_json = NULL;
    }

    int count = count_json && cJSON_IsNumber(count_json)
            ? (int)cJSON_GetNumberValue(count_json) : cJSON_GetArraySize(array);

    t_contacts *contacts_data = malloc(sizeof(t_contacts));
    if (!contacts_data) {
        logger_error("Failed to allocate memory for contacts_data\n");
        cJSON_Delete(json);
        free(response);
        return NULL;
    }
    contacts_data->count = count;
    contacts_data->contacts = malloc(sizeof(t_contact) * count);
    if (!contacts_data->contacts) {
        logger_error("Failed to allocate memory for contacts\n");
        cJSON_Delete(json);
        free(contacts_data);
        free(response);
        return NULL;
    }

    cJSON *contact = NULL;
    int i = 0;
    cJSON_ArrayForEach(contact, array) {
        cJSON *username = cJSON_GetObjectItemCaseSensitive(contact, "username");

        if (!cJSON_IsString(username)) {
            logger_error("Invalid contact data in response\n");
            for (int j = 0; j < i; j++) free(contacts_data->contacts[j].username);
            free(contacts_data->contacts);
            free(contacts_data);
            cJSON_Delete(json);
            return response;
        }

        contacts_data->contacts[i].username = mx_strdup(cJSON_GetStringValue(username));
        i++;
    }

    response->data = contacts_data;
    cJSON_Delete(json);
    return response;
}

int mx_get_contacts(long long int id, t_response **response) {
    char *request = get_request(id);
    if (!request) return -1;
    if (mx_send_data(request) < 0) {
        logger_warn("mx_get_contacts: Failed sending request");
        free(request);
        return -2;
    }
    free(request);
    char *received_data = NULL;
    if (mx_receive_data(&received_data) < 0) {
        logger_warn("mx_get_contacts: Failed receiving response");
        return -2;
    }

    *response = parse_response(received_data);
    free(received_data);
    if (!*response) return -3;
    return 0;
}
